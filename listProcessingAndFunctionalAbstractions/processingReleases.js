...
RETURN: an array of object that only contains id and title key/value pairs
RULES:
- keep only releases that have both id and title property present
- keep only id and title data for each release
ALGORITHM:
- select only releases from newReleases that have both id and title properties -> filter
- iterate through filtered releases by keys and for each key map as follows:
  - return object containing only id and title key/value pairs
*/
...
function processReleaseData(data) {
  filteredData = data.filter(pieceOfData => pieceOfData.id && pieceOfData.title)
  return filteredData.map(release => {
